...
begin
    ...
    update class_enrolled
    set attendance_rate = class_enrolled.attendance_times /(select attendance_times from class where class.id = class_enrolled.class_id)
    where class_enrolled.class_id = (select class_id from roll_call where id = NEW.roll_call_id)
      and class_enrolled.user_id = NEW.user_id;
    update roll_call
    set attendance_rate = roll_call.attendance_count /(select size from class where class.id = roll_call.class_id)
